...
async def _process_common(db: Session, user_id: str, text: str, current_state: LandDisputeState, dossier_id: Optional[str]):
    ...
    # 4. Generate Final Brief Audio if complete (overwriting previous one for this dossier)
    if updated_state.mizan_result and updated_state.mizan_result.answer_verbal:
        brief_filename = f"brief_{dossier.id}.mp3"
        brief_path = os.path.join(UPLOAD_DIR, brief_filename)
        await speak(updated_state.mizan_result.answer_verbal, brief_path)
        updated_state.mizan_result.audio_url = f"uploads/{brief_filename}"
        
        # Persist updated state with audio_url
        dossier.state = updated_state.model_dump()
        db.commit()
